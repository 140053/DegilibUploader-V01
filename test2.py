...
def t2query(barcode):
    cur.execute('SELECT * FROM public."fileStorage" WHERE filename = %s;', (barcode,))
    print(cur.fetchall())
    cur.close()


# t2query('T0006249.pdf')

# ------- POSTGRESQL FUNCTIONS

def checkduplicate(barcode):
    sql = 'SELECT * FROM public."fileStorage" WHERE filename = %s;'
    data = (barcode,)
...
def in_to_tb(metadata, val, campus, fname):
    ...
    sql1 = "INSERT INTO metadata  (title,author,taon,call_number,barcode,abstract,kurso,filename,subjek,raw_data, col,campus)VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
    val = (metadata["Title"], metadata["Author"], metadata["Year"], metadata["Info"], metadata["Barcode"], metadata["Abstract"], metadata["Course"], fname, metadata["Subject"], rawd, val, campus)
...
def intoFileStorage(data):
    ...
    sql = 'INSERT INTO public."fileStorage" (filename, data) VALUES (%s, %s);'
    cur.execute(sql, val)
